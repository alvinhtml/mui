 ...
 */
  ...
  this.context.fillRect(-1000,-1000,1000,1000);*/
	...
	topology.prototype.removeEventListener = function (event2D) {
		if (this.eventList.indexOf(event2D) != -1) this.eventList.splice(event2D, 1);
	};
	//鼠标检测函数
        ...
        tooltip: function (target) {
            ...
            switch (target.type) {
                ...
                case "vendor":
                    ...
                    break;
                case "title":
                    titleDom = '<span class="text-orange">' + target.data.name + '</span><br/>' + 'MAC：' + target.data.mac + '<br/>' + 'IP：' + target.data.ip + '<br/>' + '过期时间：' + target.data.runtime + '<br/>';
                    break;
                case "port":
                    titleDom = target.iname + '<br/>' + '管理状态：' + target.adminstate + '<br/>' + '在线状态：' + target.online + '<br/>' + '终端数量：' + target.term_num + '<br/>' + '链接状态：' + target.uplink;
                    break;
                case "term":
